...
function Login() {
  ...
  const [alertMeassage, setAlertMeassage] = useState(false);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (isValid) {
      ...
      if (!password && !email) {
        setMassageError("กรุณากรอกอีเมล และ รหัสผ่าน");
      } else if (!email.trim()) {
        setMassageError("กรุณากรอกอีเมล");
      } else if (!emailRegex.test(email.trim())) {
        setMassageError("รูปแบบอีเมลไม่ถูกต้อง");
      } else if (!password) {
        setMassageError("กรุณากรอกรหัสผ่าน");
      } else {
        setMassageError("");
      }
  ...
  return (
    <>
      <AlertMassage
        open={alertMeassage}
        setOpen={setAlertMeassage}
        title={"เข้าสู่ระบบไม่สำเร็จ"}
        massage={massageError}
      />
